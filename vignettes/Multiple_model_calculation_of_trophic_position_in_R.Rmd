...
```{r, eval = FALSE}
consumer1_models <- multiModelTP(consumer1, print = FALSE)
```
...
`multiModelsTP()` requires an object of the class `isotopeData`, which was created previously by the function `generateTPData()`. By default `multiModelsTP()` defines a `lambda = 2` for the baselines, uses 2 chains (`n.chains = 2`) to do the Bayesian calculation with 20,000 adaptive iterations (`n.adapt = 20000`), 20,000 actual iterations (`n.iter = 20000`), 20,000 iterations as burnin (`burnin = 20000`) and a thinning of 10 (`thin = 10`). Furthermore, it estimates TP using three different Bayesian models: one baseline, two baselines and two baselines full (`models = c("oneBaseline", "twoBaselines", "twoBaselinesFull"`). The default setting is to display all model diagnostics to ensure the user knows the model has converged (`print = TRUE`). To limit the amount of output for each model run (print = FALSE), however, recognize that this limited output will provided a reduced amount of information regarding model convergance . NB: The user can change any of these options, simply adding them as arguments to `multiModelTP()`, see `?multiModelTP()` for details. 
...
```{r, eval = FALSE}
consumer2_models <- multiModelTP(consumer2, print = FALSE)
```
